...
export function handleAssetCreated(event: AssetCreated): void {
  let asset = new Asset(
    event.address.toHex() + "-" + event.params.assetId_.toString()
  );
  asset.rain1155 = event.address.toHex();
  ...
  let vmStateConfig = new VMStateConfig(
    event.address.toHex() + "-" + event.params.assetId_.toString()
  );
...
export function handleTransferBatch(event: TransferBatch): void {
  ...
  while (ids && values) {
    ...
    let assetsOwned = AssetsOwned.load(
      event.address.toHex() +
        "-" +
    ...
    let asset = Asset.load(
      event.address.toHex() + "-" + id.toString()
    );
    if (asset) {
      let holders = asset.holders;
      if (holders && !holders.includes(receiver.id)) holders.push(receiver.id);
...
export function handleTransferSingle(event: TransferSingle): void {
  ...
  if (event.params.from.notEqual(ZERO_ADDRESS)) {
    ...
    sale.asset = event.address.toHex() + "-" + event.params.id.toString();
    sale.quantity = event.params.value;
