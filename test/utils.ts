...
import {
  ...
  ReserveTokenERC721__factory,
  ReserveTokenERC1155__factory,
...
export async function deployReserveToken(
  deployer?: Signer
): Promise<ReserveToken> {
  if (!deployer) {
    deployer = (await ethers.getSigners())[0];
  }
  return await (await new ReserveToken__factory(deployer).deploy()).deployed();
}
...
export async function deployToken(
  ...
  return await (
    await new Token__factory(deployer).deploy(name, symbol)
  ).deployed();
}
...
export async function deployErc721(
  ...
  return await (
    await new ReserveTokenERC721__factory(deployer).deploy(name, symbol)
  ).deployed();
}
...
export async function deployErc1155(
  ...
  return await (
    await new ReserveTokenERC1155__factory(deployer).deploy()
  ).deployed();
}

export async function deployERC20BalanceTier(
  args: ERC20BalanceTierConfigStruct,
  owner?: Signer
) {
  ...
  const tx = await erc20BalanceTierFactory.createChildTyped(args);

  if (!owner) {
    owner = (await ethers.getSigners())[0];
  }

  const tier = new ERC20BalanceTier__factory(owner).attach(
    await getChild(erc20BalanceTierFactory, tx)
  );

  return await tier.deployed();
}
